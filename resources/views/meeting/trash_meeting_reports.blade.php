...
<li><a href="{{ route('restoreAllMeetingReports') }}" class="btn btn-success btn btn-sm">
      ...
      <td class="text-center">{{$reportsList->deleted_at}}</td>
	      <td class="text-center">
			<ul class="list-unstyled list-inline">
				...
				<li> <a class="btn btn-primary btn btn-sm" href="{{'/meeting/'.$reportsList->id.'/restore'}}">
	           		<i class="glyphicon glyphicon-ok" aria-hidden="true"></i> Restore
	           	</a>
	          	</li>
	          	
	          	<!-- <li>
					<form action="{{'/meeting/'.$reportsList->id}}/delete" method="post" class="form-group">
		            {{csrf_field()}}
		            {{method_field('DELETE')}}
		            <input type="submit" value="Delete" class="btn btn-sm btn-danger  glyphicon glyphicon-remove" onclick="return confirmDelete();">
		            </form>
	          </li> -->
			</ul>      	
	      </td>
