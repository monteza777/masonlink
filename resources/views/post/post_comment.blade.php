...
<div class="row">
<div class="col-md-12">
<form action="/post/{{$posthead->id}}/store" method="post">
 {{csrf_field()}}
<div class="col-md-12">
...
<div class="comment">
	<!-- <h3 class="comment-title"><span class="glyphicon glyphicon-comment"></span> {{$posthead->comments()->count()}} 
	{{ $posthead->comments()->count() === 1 || $posthead->comments()->count() == 0? "Comment": "Comments" }}
	</h3>
	@foreach($posthead->comments as $comment)
	<div class="author-info">
		<img src="" class="author-image">
		<div class="author-name">
		<h4 class="author-name">Member Comment</h4>
		<p class="author-time">{{date('M j, Y h:ia', strtotime($posthead->created_at))}}</p>
		</div>
	</div>
	<div class="comment-content">
		<p>- {{$comment->comment}}</p>
	...
	@endforeach
	 ...
	 <table class="table">
	 	...
	 	<tr>
	 		...
	 		<td>
	 		<a href="3" class="btn btn-primary btn-xs glyphicon glyphicon-edit"></a>
